...
def get_prompt(player, memory_size, rules, question = "Answer saying which action Player 1 should play."):
  if question == "Answer saying which action Player 1 should play.":
    assistant_text =  "\n\n{'value': '"
    if "QwQ" in config.model.model_name or "deepseek-ai/DeepSeek-R1" in config.model.model_name:
      assistant_text =  "<think>\n{'value': '"
  else:
    assistant_text =  "\n\n{'value': "

  if "gpt-4o" in config.model.model_name:
    assistant_text =  "{'value': '"
    assistant_prompt = {"role": "assistant", "content": assistant_text}
    system_prompt = {'role': "developer", "content": get_system_prompt(player, memory_size, rules)}
